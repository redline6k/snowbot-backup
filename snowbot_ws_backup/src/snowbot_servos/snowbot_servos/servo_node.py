...
class SnowbotNode(Node):
    def __init__(self):
        ...
        self.create_subscription(
            Float64, "/led_control", self.led_cb, 10
        )  # NEW: LED subscriber
        ...
        self.get_logger().info(
            "Lift + Continuous Chute + Auger ESC + LEDs with soft start READY"
        )
    ...
    def servo_cb(self, msg):
        ...
        if chute_raw != -999:
            ...
            self.get_logger().info(
                f"Chute command received: {self.last_chute_command:.2f}"
            )
            self.update_chute()
    ...
    def led_cb(self, msg):
        # HW-482 is High-Level Trigger
        # 0.0 or less = OFF
        # Greater than 0.0 = ON
        if msg.data > 0.0:
            # Full duty cycle (65535) provides a constant HIGH signal
            self.kit._pca.channels[7].duty_cycle = 0xFFFF
            self.get_logger().info("LED Relay: ON")
        else:
            # 0 duty cycle provides a constant LOW signal
            self.kit._pca.channels[7].duty_cycle = 0x0000
            self.get_logger().info("LED Relay: OFF")
...
def main():
    ...
    try:
        ...
        node.kit.servo[7].angle = 0  # NEW: Turn off LEDs on shutdown
        GPIO.cleanup()
        rclpy.shutdown()
